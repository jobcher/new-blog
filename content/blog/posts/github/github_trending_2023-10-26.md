...
#### 侯孝贤罹患痴呆症，已结束他的电影生涯。英国影评人安东尼·雷恩斯10月23日于《童年往事》在伦敦的放映活动中透露了该消息，IndieWire随后向侯孝贤的身边人求证...
<p>侯孝贤罹患痴呆症，已结束他的电影生涯。英国影评人安东尼·雷恩斯10月23日于《童年往事》在伦敦的放映活动中透露了该消息，IndieWire随后向侯孝贤的身边人求证属实。不过，侯孝贤仍在黄文英执导、下月上映的《车顶上的玄天上帝》中署名监制。<br>侯孝贤在台北的工作室已关闭，但他和家人尚未就侯的健康状况作过宣布，相信是希望保持隐私。<br>侯孝贤1947年生，与杨德昌等同属80年代台湾新电影的代表人物。<br>（<a href="https://www.indiewire.com/news/general-news/hou-hsiao-hsien-retired-from-filmmaking-dementia-1234919475/" target="_blank" rel="noopener" onclick="return confirm('Open this link?\n\n'+this.href);">IndieWire</a>）</p>

#### ↩️ 使用的话，直接在后面跟复制下来的b站分享文本，比如python get_url.py加个空格把复制的分享文本粘贴就行。 re库是之前想处理文本多出来的标题使用的，后来...
<div class="rsshub-quote"><blockquote>                                    <p><a href="https://t.me/DNSPODT/2081"><b>LoopDNS资讯播报</b>:</a></p>                                                                        <p>import requests import re import sys  def get_final_url(short_url):     try:         # 允许重定向         response = requests.head(short_url, allow_redirects=True)         # 返回长链接         return response.url     except requests.exceptions.RequestException as e:…</p>                                </blockquote></div><p>使用的话，直接在后面跟复制下来的b站分享文本，比如python get_url.py加个空格把复制的分享文本粘贴就行。<br><br>re库是之前想处理文本多出来的标题使用的，后来发现带空格，因此直接取命令行最后一个参数就是短链接，可直接把顶部对re库的引入给去掉</p>

#### ↩️ import requests import re import sys def get_final_url(short_url): try: # 允许重定向 response = requests.head(short_url, allow_redirects=True) # ...
<div class="rsshub-quote"><blockquote>                                    <p><a href="https://t.me/DNSPODT/2080"><b>LoopDNS资讯播报</b>:</a></p>                                                                        <p>短链接跳转主要是http 301/302，将用户重定向至另一个网址。主流语言的httpclient允许跳转后即可获得最终的长链接。  频道编辑在此处提供一个31行的python小程序，可以直接复制b站app的短链接使用，作为命令行参数传入，不需要删除文本中的标题，可以同时去除长链接中的追踪参数。如有兴趣的可参考代码集成到自己的服务中。  代码将粘贴到评论区</p>                                </blockquote></div><p><code>import requests<br>import re<br>import sys<br><br>def get_final_url(short_url):<br>    try:<br>        # 允许重定向<br>        response = requests.head(short_url, allow_redirects=True)<br>        # 返回长链接<br>        return response.url<br>    except requests.exceptions.RequestException as e:<br>        # 处理异常<br>        print(f"请求异常: {e}")<br>        return None<br><br>if __name__ == "__main__":<br>    if len(sys.argv) == 1:<br>        print("请输入参数")<br>    else:        <br>        short_url=sys.argv[2]<br>        final_url = get_final_url(short_url)<br>        if final_url:            <br>            mark_index = final_url.find("?")<br>            if mark_index != -1:           <br>              cleaned_url = final_url[:mark_index]<br>              print(cleaned_url)<br>            else:            <br>              cleaned_url = url<br>              print(cleaned_url)<br>        else:<br>            print("无法获取最终URL")</code></p>

#### 短链接跳转主要是http 301/302，将用户重定向至另一个网址。主流语言的httpclient允许跳转后即可获得最终的长链接。 频道编辑在此处提供一个31行的python小程序...
<p>短链接跳转主要是http 301/302，将用户重定向至另一个网址。主流语言的httpclient允许跳转后即可获得最终的长链接。<br><br>频道编辑在此处提供一个31行的python小程序，可以直接复制b站app的短链接使用，作为命令行参数传入，不需要删除文本中的标题，可以同时去除长链接中的追踪参数。如有兴趣的可参考代码集成到自己的服务中。<br><br>代码将粘贴到评论区</p>

#### ↩️🖼 浪人新闻管理［小偷阿晶］疑似因使用带跟踪的BiliBili分享链接被警方带走，其Telegram账号已注销 Telegram 知名频道 浪人新闻 管理 [小偷阿晶] 当前疑...
<div class="rsshub-quote"><blockquote>                                    <p><a href="https://t.me/DNSPODT/2077"><b>LoopDNS资讯播报</b>:</a></p>                                                                        <p>如果有心的人想探寻知乎这个“宝库”，可能会怎么做呢？  假设知乎记录分享链接点击日志的表有5个主要的字段，分别是user_id（点击链接的人的用户id），item id（帖子的id），item_type（根据知乎链接的mvc推断回答和想法的类型是分开的），utm_source（分享链接的来源，看引用的上方）和utm_oi（分享用户的唯一标识）。  首先使用WHERE语句将telegram的包名过滤出来，比如  WHERE utm_source=‘org.telegram.messenger’  然后使用降序将对utm_oi排序…</p>                                </blockquote></div><p><b>浪人新闻管理［小偷阿晶］疑似因使用带跟踪的BiliBili分享链接被警方带走，其Telegram账号已注销</b><br><br>Telegram 知名频道 <b>浪人新闻</b> 管理 <b>[小偷阿晶]</b> 当前疑似被警方带走，<b>目前他的Telegram 账号处于已注销状态</b>，此前曾高调宣称自己发布的 <a href="http://b23.tv/" target="_blank" rel="noopener"><b>b23.tv</b></a> 短链接是<b>安全</b>的，然而仅仅过了几天，他便被警方拘留。目前，他的Telegram账号已经被注销，具体情况仍在进一步调查中。</p><img src="https://cdn5.telegram-cdn.org/file/YIDU5vklPf0lI0ogDegGBVbJE5tfS1ZUHW9lqOswbMNuqKjcYFI-7CbyliQXFkV7buyjuX4T6644NkX7-lAd48MHJYIIbl2NEyW2PwY2rdSu6-KltMHvin6cgWIb0UIo72amfd0dn3NscRJWVGm9bl4AwLulZIZaNpF2072BxjtujxxfQqdvvMTSEPbvljYe01c_JiA05AxroXA5I2wXUQsxNAXavOEXdJ7PVXW3AMTeGhzDC7KNd9iUWqdBdaaNMyrxXVUoAbp7as1e9DKbWE9ruKxYSE9ps8oqRB74j55Q_JPkpKYYhkyWZWfgSq-alY0sDlADQtS_DjEseGg6mA.jpg" referrerpolicy="no-referrer">

#### 在中国被关押超过三年的澳大利亚记者成蕾已于本月中旬返澳，她在近日接受了ABC专访。被捕前，成蕾不认为自己会成为中国国家安全机构的目标。被捕数月后，她被指...
<p>在中国被关押超过三年的澳大利亚记者成蕾已于本月中旬返澳，她在近日接受了ABC专访。被捕前，成蕾不认为自己会成为中国国家安全机构的目标。被捕数月后，她被指控向境外非法提供国家机密。她说，指控的核心是她向一名记者透露了一份未解除发稿禁令的政府报告。<br><br><a href="https://www.abc.net.au/news/2023-10-17/cheng-lei-breaks-her-silence-first-interview-china-jail/102988438" target="_blank" rel="noopener" onclick="return confirm('Open this link?\n\n'+this.href);">ABC News</a></p>

#### ↩️ 如果有心的人想探寻知乎这个“宝库”，可能会怎么做呢？ 假设知乎记录分享链接点击日志的表有5个主要的字段，分别是user_id（点击链接的人的用户id），item...
<div class="rsshub-quote"><blockquote>                                    <p><a href="https://t.me/DNSPODT/2073"><b>LoopDNS资讯播报</b>:</a></p>                                                                        <p>【温馨提示】安卓用户请勿使用知乎一键分享功能将内容分享到telegram。  知乎分享调用的并不是系统的api，而是自行编写的组件，因此可以获取用户的分享目标。在获取到目标包名后会拼接到链接中utm_source这个参数中。  这个追踪参数可能会长期保存在知乎数据库中。比如您选择分享到telegram上，可能在确认的那一刻telegram的包名已经发送并保存到知乎数据库中，而后续点击您分享链接的人也会被关联。因此，如果您需要分享知乎内容，请在分享界面选择复制链接并留意删除链接中的追踪参数。</p>                                </blockquote></div><p>如果有心的人想探寻知乎这个“宝库”，可能会怎么做呢？<br><br>假设知乎记录分享链接点击日志的表有5个主要的字段，分别是user_id（点击链接的人的用户id），item id（帖子的id），item_type（根据知乎链接的mvc推断回答和想法的类型是分开的），utm_source（分享链接的来源，看引用的上方）和utm_oi（分享用户的唯一标识）。<br><br>首先使用WHERE语句将telegram的包名过滤出来，比如<br><br><code>WHERE utm_source=‘org.telegram.messenger’<br><br></code>然后使用降序将对utm_oi排序<br><br></p><pre>ORDER BY utm_oi DESC<br><br></pre>（勘误：要去重获取不同的标识符，然后逐条标识符count算总数之后才能排序，太久没写sql了）<br><br>这样就可以将点击量最高的分享到telegram的某个用户给筛选出来。<br><br>当然了现实里的系统不会那么简单，但是思路都是差不多。<br><br>分享功能并不会带来危险，风险性来源于telegram这个软件<p></p>

#### ↩️ 进一步测试可得 1、知乎分享链接有3个主要参数：分别是utm_source、utm_medium和utm_oi。 其中第一个代表点击来源，第二个暂且不明，第三个是用户的唯一标...
<div class="rsshub-quote"><blockquote>                                    <p><a href="https://t.me/DNSPODT/2073"><b>LoopDNS资讯播报</b>:</a></p>                                                                        <p>【温馨提示】安卓用户请勿使用知乎一键分享功能将内容分享到telegram。  知乎分享调用的并不是系统的api，而是自行编写的组件，因此可以获取用户的分享目标。在获取到目标包名后会拼接到链接中utm_source这个参数中。  这个追踪参数可能会长期保存在知乎数据库中。比如您选择分享到telegram上，可能在确认的那一刻telegram的包名已经发送并保存到知乎数据库中，而后续点击您分享链接的人也会被关联。因此，如果您需要分享知乎内容，请在分享界面选择复制链接并留意删除链接中的追踪参数。</p>                                </blockquote></div><p>进一步测试可得<br><br>1、知乎分享链接有3个主要参数：分别是utm_source、utm_medium和utm_oi。<br><br>其中第一个代表点击来源，第二个暂且不明，第三个是用户的唯一标识符。<br><br>2、utm_oi即使在不登陆的情况下分享依然存在。而登陆后同一设备下此参数会变更，每一条分享都相同。因此证明此项为用户标识符。<br><br>3、从第二点合理怀疑，在点击确定分享时相关数据已经上传到服务器。</p>

#### 🖼 2024年放假安排：除夕不放假，春节连休8天 根据中国政府网消息，2024年元旦、春节、放假调休日期的具体安排如下： 元旦：1月1日放假，与周末连休。 春节：2...
<p><b>2024年放假安排：除夕不放假，春节连休8天</b><br><br>根据中国政府网消息，2024年元旦、春节、放假调休日期的具体安排如下：<br><br>元旦：1月1日放假，与周末连休。<br><br>春节：2月10日至17日放假调休，共8天。2月4日（星期日）、2月18日（星期日）上班。鼓励各单位结合带薪年休假等制度落实，安排职工在除夕（2月9日）休息。<br><br>来源： <a href="https://www.gov.cn/zhengce/content/202310/content_6911527.htm" target="_blank" rel="noopener" onclick="return confirm('Open this link?\n\n'+this.href);">中国政府网</a></p><img src="https://cdn5.telegram-cdn.org/file/HUP8doLhqUoiTOhqDohlH01NxQ38xRfBzFWcKZ6EI5_hqsfwgceETfs73arsvzHznLr-AyzvRuYxdudQt8qWYjdv1jmY8SU4jzCzf9tE